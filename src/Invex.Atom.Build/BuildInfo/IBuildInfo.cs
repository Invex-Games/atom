namespace Invex.Atom.Build.BuildInfo;
...
public interface IBuildInfo : IBuildAccessor
{
    ...
    private string DefaultBuildName
    {
        get
        {
            var solutionFile = RootedFileSystem
                                   .Directory
                                   .GetFiles(RootedFileSystem.AtomRootDirectory,
                                       "*.slnx",
                                       SearchOption.TopDirectoryOnly)
                                   .FirstOrDefault() ??
                               RootedFileSystem
                                   .Directory
                                   .GetFiles(RootedFileSystem.AtomRootDirectory, "*.sln", SearchOption.TopDirectoryOnly)
                                   .FirstOrDefault();

            Logger.LogDebug("Determined solution file: {SolutionFile}", solutionFile);

            return solutionFile is not null
                ? new RootedPath(RootedFileSystem, solutionFile).FileNameWithoutExtension
                : RootedFileSystem
                      .AtomRootDirectory
                      .DirectoryName
                      ?.Split(RootedFileSystem.Path.DirectorySeparatorChar,
                          RootedFileSystem.Path.AltDirectorySeparatorChar)[^1] ??
                  "Unknown";
        }
    }
