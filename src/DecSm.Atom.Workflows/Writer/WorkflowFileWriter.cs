...
/// <param name="atomFileSystem">The file system service for file operations.</param>
...
/// </remarks>
[PublicAPI]
public abstract class WorkflowFileWriter<T>(IAtomFileSystem atomFileSystem, ILogger<WorkflowFileWriter<T>> logger)
    : IWorkflowWriter<T>
    where T : IWorkflowType
{
    ...
    /// </summary>
    protected virtual RootedPath FileLocation => atomFileSystem.AtomRootDirectory;
    ...
    public async Task Generate(WorkflowModel workflow, CancellationToken cancellationToken = default)
    {
        ...
        if (!atomFileSystem.Directory.Exists(FileLocation))
            atomFileSystem.Directory.CreateDirectory(FileLocation);

        await atomFileSystem.File.WriteAllTextAsync(filePath, newText, cancellationToken);
    }
