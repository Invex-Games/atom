...
public interface IDotnetPublishHelper : IDotnetCliHelper, IBuildInfo
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var buildDirectory = options.PublishOptions?.Output is { Length: > 0 }
            ? RootedFileSystem.CreateRootedPath(options.PublishOptions?.Output!)
            : RootedFileSystem.AtomRootDirectory / projectName / configuration / "atom-publish";
        ...
        if (RootedFileSystem.Directory.Exists(publishDirectory))
        {
        ...
        if (!RootedFileSystem.Directory.Exists(publishDirectory.Parent!))
            RootedFileSystem.Directory.CreateDirectory(publishDirectory.Parent!);
