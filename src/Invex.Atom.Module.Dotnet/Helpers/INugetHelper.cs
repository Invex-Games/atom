...
///     and provides functionality for common NuGet operations within a Invex.Atom build.
/// </remarks>
[PublicAPI]
public interface INugetHelper : IBuildInfo
{
    ...
    RootedPath NugetConfigPath
    {
        get
        {
            ...
            var appDataPath =
                RootedFileSystem.CreateRootedPath(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData));
    ...
    {
        var packageBuildDir = RootedFileSystem.AtomArtifactsDirectory / projectName;
        var packages = RootedFileSystem.Directory.GetFiles(packageBuildDir, "*.nupkg");
        ...
        CancellationToken cancellationToken = default)
    {
        if (skipIfExists)
        {
            var nugetContents = await RootedFileSystem.File.ReadAllTextAsync(NugetConfigPath, cancellationToken);
