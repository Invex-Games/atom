...
public partial interface IDotnetTestHelper : IDotnetCliHelper, IBuildInfo, IDotnetToolInstallHelper, IReportsHelper
{
    ...
    {
        var projectPath = DotnetFileUtil.GetProjectFilePathByName(RootedFileSystem, projectName) ??
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        testOptions = testOptions with
        {
            Output = testOptions.Output is { Length: > 0 }
                ? testOptions.Output
                : RootedFileSystem.AtomRootDirectory / projectName / "TestResults",
        ...
        var testOutputDirectory = RootedFileSystem.CreateRootedPath(testOptions.Output);

        var publishDirectory = RootedFileSystem.AtomPublishDirectory / projectName;

        var testResultsPublishDirectory = publishDirectory / "test-results";

        if (RootedFileSystem.Directory.Exists(testResultsPublishDirectory))
            RootedFileSystem.Directory.Delete(testResultsPublishDirectory, true);

        RootedFileSystem.Directory.CreateDirectory(testResultsPublishDirectory);

        var coverageResultsPublishDirectory = publishDirectory / "coverage-results";

        if (options.IncludeCoverage && RootedFileSystem.Directory.Exists(coverageResultsPublishDirectory))
            RootedFileSystem.Directory.Delete(coverageResultsPublishDirectory, true);
        ...
        if (RootedFileSystem.Directory.Exists(testOutputDirectory))
        {
            ...
            RootedFileSystem.Directory.Delete(testOutputDirectory, true);
        }
        ...
        await using var transformFilesScope =
            (options.SetVersionsFromProviders, options.CustomPropertiesTransform) switch
            {
                (true, not null) => await TransformProjectVersionScope
                    .CreateAsync(
                        DotnetFileUtil.GetPropertyFilesForProject(projectPath, RootedFileSystem.AtomRootDirectory),
                        BuildVersion,
                        cancellationToken)
        ...
        if (result.ExitCode is not 0 && !RootedFileSystem.File.Exists(trxFile))
        ...
        RootedFileSystem.File.Copy(testOutputDirectory / $"{projectName}.html",
            testResultsPublishDirectory / $"{projectName}.html");

        GenerateTestReport(projectName, testOptions.Configuration, testOptions.Framework, trxFile);
        ...
        await DotnetCli.ToolExecute("dotnet-reportgenerator-globaltool",
            [
                ...
                "-sourcedirs:" + RootedFileSystem.AtomRootDirectory,
            ],
            new()
            {
                Yes = true,
    ...
    void GenerateCoverageReport(
        ...
        bool includeTitle = true)
    {
        var coverageJson = RootedFileSystem.File.ReadAllText(coverageJsonFile);
