...
public interface IDotnetPackHelper : IDotnetCliHelper, IBuildInfo
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var buildDirectory = options.PackOptions?.Output is { Length: > 0 }
            ? RootedFileSystem.CreateRootedPath(options.PackOptions?.Output!)
        ...
        if (RootedFileSystem.Directory.Exists(buildDirectory))
        {
            Logger.LogDebug("Deleting existing pack directory {PackDirectory}", buildDirectory);
            RootedFileSystem.Directory.Delete(buildDirectory, true);
        }
        ...
        var packagedFile = RootedFileSystem.CreateRootedPath(RootedFileSystem
            .Directory
        ...
        if (options.ClearPublishDirectory && RootedFileSystem.Directory.Exists(publishDirectory))
        {
            Logger.LogDebug("Deleting existing publish directory {PublishDirectory}", publishDirectory);
            RootedFileSystem.Directory.Delete(publishDirectory, true);
        }

        RootedFileSystem.Directory.CreateDirectory(publishDirectory);

        if (RootedFileSystem.File.Exists(publishedFile))
        {
            Logger.LogDebug("Deleting existing published file {PublishedFile}", publishedFile);
            RootedFileSystem.File.Delete(publishedFile);
        }

        RootedFileSystem.File.Move(packagedFile, publishedFile);
