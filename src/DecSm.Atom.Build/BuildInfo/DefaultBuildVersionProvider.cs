...
/// </remarks>
/// <param name="atomFileSystem">The file system service for accessing project files.</param>
internal sealed partial class DefaultBuildVersionProvider(IAtomFileSystem atomFileSystem) : IBuildVersionProvider
{
    /// <summary>
    ...
    public SemVer Version
    {
        get
        {
            var directoryBuildProps = atomFileSystem.AtomRootDirectory / "Directory.Build.props";

            if (!directoryBuildProps.FileExists)
                return SemVer.One;

            var contents = atomFileSystem.File.ReadAllText(directoryBuildProps);
