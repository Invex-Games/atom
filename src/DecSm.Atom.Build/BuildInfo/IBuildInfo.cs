namespace DecSm.Atom.Build.BuildInfo;
...
public interface IBuildInfo : IBuildAccessor
{
    ...
    private string DefaultBuildName
    {
        get
        {
            var solutionFile = AtomFileSystem
                                   .Directory
                                   .GetFiles(AtomFileSystem.AtomRootDirectory, "*.slnx", SearchOption.TopDirectoryOnly)
                                   .FirstOrDefault() ??
                               AtomFileSystem
                                   .Directory
                                   .GetFiles(AtomFileSystem.AtomRootDirectory, "*.sln", SearchOption.TopDirectoryOnly)
                                   .FirstOrDefault();

            Logger.LogDebug("Determined solution file: {SolutionFile}", solutionFile);

            return solutionFile is not null
                ? new RootedPath(AtomFileSystem, solutionFile).FileNameWithoutExtension
                : AtomFileSystem
                      .AtomRootDirectory
                      .DirectoryName
                      ?.Split(AtomFileSystem.Path.DirectorySeparatorChar,
                          AtomFileSystem.Path.AltDirectorySeparatorChar)[^1] ??
                  "Unknown";
        }
    }
