...
internal sealed class ReportLogger(IServiceProvider serviceProvider, IExternalScopeProvider? scopeProvider) : ILogger
{
    private IParamService ParamService => field ??= serviceProvider.GetRequiredService<IParamService>();

    private ReportService ReportService => field ??= serviceProvider.GetRequiredService<ReportService>();
        ...
        Func<TState, Exception?, string> formatter)
    {
        ...
        ReportService.AddReportData(new LogReportData(message, exception, logLevel, time), command);
    }
}
