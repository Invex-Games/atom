namespace DecSm.Atom.Build.Variables;

/// <summary>
///     The default implementation of <see cref="IVariableProvider" />, which uses a local JSON file for storage.
/// </summary>
/// <remarks>
...
/// </remarks>
/// <param name="atomFileSystem">The file system service for accessing the variables file.</param>
/// <param name="buildModel">The build model for accessing the current target context.</param>
internal sealed partial class AtomVariableProvider(IAtomFileSystem atomFileSystem, BuildModel buildModel)
    : IVariableProvider
{
    /// <summary>
    ...
    {
        var variablesPath = atomFileSystem.AtomTempDirectory / "variables";

        Dictionary<string, string>? variables;

        if (variablesPath.FileExists)
        {
            var text = await atomFileSystem.File.ReadAllTextAsync(variablesPath, cancellationToken);
        ...
        await atomFileSystem.File.WriteAllTextAsync(variablesPath, json, cancellationToken);
