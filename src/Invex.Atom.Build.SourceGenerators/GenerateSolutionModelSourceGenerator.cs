...
#pragma warning restore RS1035
        ...
        ImmutableDictionary<string, string> projects)
    {
        ...
        var projectFileTypeLines = string.Join("\n\n",
            projects.Select(kvp =>
            {
                ...
                return $$"""
                             ...
                             ///    {{projectPath}}
                             /// </summary>
                             public interface {{validIdentifier}} : IPathMarker
                             {
                                 public const string Name = @"{{kvp.Key}}";

                                 static RootedPath IPathMarker.Path(IFileSystem fileSystem) =>
                                     new RootedPath(fileSystem, @"{{projectPath}}");

                                 new static RootedPath Path(IFileSystem fileSystem) =>
                                     new RootedPath(fileSystem, @"{{projectPath}}");
                             }
                         """;
            }));
        ...
        var code = $$"""
                     ...
                     /// </summary>
                     public interface Solution : IPathMarker
                     {
                         public const string Name = @"{{solutionName}}";

                         static RootedPath IPathMarker.Path(IFileSystem fileSystem) =>
                            new RootedPath(fileSystem, @"{{solutionPathNormalized}}");
                     }
